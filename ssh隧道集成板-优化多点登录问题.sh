...
# FIX: 修复了多点登录时因 Flask 路由名称错误导致的 Internal Server Error (P9)。
# FIX: 修复了 HAS_CRYPT 变量未定义的 NameError (P10)。
# ==========================================================
...
apt update -y
# 确保安装 procps（用于 psutil, ps -S 等）和 libffi-dev
...
if [ ! -f "$ROOT_HASH_FILE" ] && [ -n "${PANEL_ROOT_PASS_RAW:-}" ]; then
    if [ "$HAS_BCRYPT" -eq 1 ]; then
        ...
        # P2 修复：回退到带盐的 SHA-512 crypt hash (更安全的回退)
...
# =============================
# BBR 拥塞控制和网络调优 (增加 TCP Keep-Alive 优化)
# =============================
...
# NEW: 增加 TCP Keep-Alive 探测频率和次数，防止中间节点断开空闲连接
...
# WSS 脚本：使用 Python 占位符或硬编码常量，不依赖 Bash 替换内部变量
...
BUFFER_SIZE = 65536
TIMEOUT = 86400  # <--- 修改: 从 3600s (1h) 增加到 86400s (24h)
...
# 创建 WSS systemd 服务 (ExecStart 传入端口参数和内部转发端口参数)
...
# =============================
# Traffic Control 基础配置 (D. 环境与配置优化)
# =============================
...
if [ -z "$IP_DEV" ]; then
    ...
    # D. 环境与配置优化: 清除所有现有的 tc 规则，确保环境干净
...
# =============================
# WSS 用户管理面板 (Python/Flask) - 核心逻辑 (P9, P10 修复)
# =============================
echo "==== 重新部署 WSS 用户管理面板 (Python/Flask) V2.1 (P9, P10 修复) ===="
...
def get_service_logs(service):
    ...
    try:
        command = [shutil.which('journalctl') or '/bin/journalctl', '-u', service, f'-n', '50', '--no-pager', '--utc']
        success, output = safe_run_command(command)
        return output if success else f"错误: 无法获取 {service} 日志. {output}"
    except Exception as e:
...
def manage_quota_iptables_rule(username, uid, action='add', quota_limit_bytes=0):
    ...
    try:
        ...
        while True:
            # 尝试删除带配额的规则
            result = subprocess.run([iptables_bin, '-t', 'filter', '-D', QUOTA_CHAIN] + base_rule + ['-m', 'quota', '--quota', '0', '-j', 'RETURN'], capture_output=True, timeout=2)
            if result.returncode != 0: break
        while True:
            # 尝试删除 DROP 规则
            result = subprocess.run([iptables_bin, '-t', 'filter', '-D', QUOTA_CHAIN] + base_rule + ['-j', 'DROP'], capture_output=True, timeout=2)
            if result.returncode != 0: break
        while True:
            # 尝试删除无 quota 的 RETURN 规则 (作为无限流量的计数规则)
            result = subprocess.run([iptables_bin, '-t', 'filter', '-D', QUOTA_CHAIN] + base_rule + ['-j', 'RETURN'], capture_output=True, timeout=2)
            if result.returncode != 0: break
    except Exception:
        pass # 忽略清理错误
...
def sync_user_status(user):
    ...
    if user['status'] != 'exceeded' and quota_limit_bytes > 0:
        ...
        manage_quota_iptables_rule(username, uid, 'add', 0) # Infinite usage, still count traffic
    else:
        # 如果是 exceeded 状态，我们不添加 RETURN 规则，确保流量被后续的 DROP 规则捕获
        manage_quota_iptables_rule(username, uid, 'delete', 0) 
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">用量/限额 (GB)</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">限速 (KB/s)</th>
    ...
    <script>
        ...
        function renderPortStatusList(ports) {
            ...
            ports.forEach(p => {
                ...
                container.innerHTML += 
                    '<div class="flex justify-between items-center text-gray-700 p-2 bg-white border border-gray-100 rounded-lg shadow-sm">' +
                ...
                tbody.innerHTML += 
                    '<tr id="row-' + user.username + '" class="hover:bg-gray-50 transition duration-100">' +
                        ...
                        '<td class="px-6 py-4 whitespace-nowrap text-sm">' +
                            ...
                            '</span>' +
                        '</td>' +
                        '<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">' + (user.expiration_date || '永不') + '</td>' +
                        // NEW: 连接数
        ...
        function renderAuditLogs(logs) {
            ...
            filteredLogs.forEach(log => {
                ...
                if (parts) {
                    ...
                    // FIX: 使用 createElement 和 textContent 来防止 XSS
                    const detailsDiv = document.createElement('div');
                    detailsDiv.textContent = details;
                    const safeDetails = detailsDiv.innerHTML;

                    logContainer.innerHTML += 
                        '<div class="text-xs text-gray-700 font-mono space-y-1 p-1 hover:bg-gray-200 rounded-md">' +
                            '<span class="text-indigo-600">' + timestamp.split(' ')[1] + '</span> ' +
                            '<span class="font-bold">[' + user + ']</span> ' +
                            '<span class="text-sm font-semibold text-gray-900">' + action + '</span> ' +
                            '<span class="text-gray-500">' + safeDetails + '</span>' + 
                        '</div>';
                } else {
                    // 如果格式不匹配，直接显示原始日志（已通过 textContent 保护）
                    const tempDiv = document.createElement('div');
                    tempDiv.textContent = log;
                    logContainer.innerHTML += '<div class="text-xs text-gray-700 font-mono p-1">' + tempDiv.innerHTML + '</div>';
                }
            });
        }
...
echo "WSS 管理面板 V2.1 (P9, P10 修复) 已启动，端口 $PANEL_PORT"
echo "----------------------------------"
...
echo "Web 面板用户名: root"
echo "Web 面板密码: [您设置的密码]"
echo ""
echo "--- 修复详情 (P9, P10) ---"
echo "修复了多点登录时因 Flask 路由名称错误导致的 BuildError (500)。"
echo "修复了由于 Python 模块导入失败导致 HAS_CRYPT 变量未定义的 NameError。"
echo "现在面板应该能稳定运行并处理密码回退校验。"
echo "=================================================="
